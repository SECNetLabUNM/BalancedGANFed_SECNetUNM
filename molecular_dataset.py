...
class MolecularDataset():
    ...
    def loadNoRandom(self, filename):
    ...
    def generate(self, filename, add_h=False, filters=lambda x: True, size=None, validation=0.1, test=0.1):
        ...
        if filename.endswith('.pkl'):
            with open(filename, 'rb') as f:
                self.data = [Chem.MolFromSmiles(line) for line in pickle.load(f)]
    ...
    def _genF(self, mol, max_length=None):
        ...
        features = np.array([[*[a.GetDegree() == i for i in range(5)], 
                              *[a.GetExplicitValence() == i for i in range(10)],
                              *[int(a.GetHybridization()) == i for i in range(1, 7)], 
                              *[a.GetImplicitValence() == i for i in range(10)], 
                              a.GetIsAromatic(),
                              a.GetNoImplicit(),
...
def data_noniid(dataset, num_users, path='data_smiles/esol_smiles.pkl'):
    """
    ...
    mol_data.generate(path, validation=0.1, test=0.1)
    data = mol_data.data
    ...
    for i in range(num_users):
        ...
        for rand in rand_set:
            dict_users[i] = np.concatenate(
                (dict_users[i], idxs[rand*num_mols:(rand+1)*num_mols]), axis=0)
    return dict_users
