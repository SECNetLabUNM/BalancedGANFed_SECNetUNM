...
class Trainer(object):
    ...
    def tnr(self, modeld, modelg, global_round, progressDictLocal={}):
        ...
        self.V.load_state_dict(modeld.state_dict())
        ...
        for i in range(start_iters, self.num_iters_local):
            ...
            if (i+1) % self.log_step == 0:
                mols, _, _, a, x, _, _, _, _ = self.data.next_validation_batch() ; isValid,opName = True, "next_validation_batch"
                z = self.sample_z(a.shape[0])
                print('[Valid]', '')
            else:
                mols, _, _, a, x, _, _, _, _ = self.data.next_train_batch(self.batch_size) ; isValid,opName = False, "next_train_batch"
            ...
            progressDictLocal[i]['type(a)', opName] = str( type(a) )
            progressDictLocal[i]['type(x)', opName] = str( type(x) )
            progressDictLocal[i]['a.shape', opName] = str( a.shape )
            progressDictLocal[i]['x.shape', opName] = str( x.shape )
            a = torch.from_numpy(a).to(self.device).long()            # Adjacency.
            x = torch.from_numpy(x).to(self.device).long()            # Nodes.
            progressDictLocal[i]['a = torch.from_numpy(a).to(self.device).long()'] = str( type(a) )
            progressDictLocal[i]['x = torch.from_numpy(x).to(self.device).long()'] = str( type(x) )
            a_tensor = self.label2onehot(a, self.b_dim)
            x_tensor = self.label2onehot(x, self.m_dim)
            progressDictLocal[i]['a_tensor'] = str( type(a_tensor) )
            progressDictLocal[i]['x_tensor'] = str( type(x_tensor) )
